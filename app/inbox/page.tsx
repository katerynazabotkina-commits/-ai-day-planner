...
function TaskCard({ task }: { task: Task }) {
  ...
  return (
    <div
      className={`flex items-start gap-3 px-4 py-4 bg-white rounded-2xl border transition-all ${
    ...
    >
      ...
      <button
        ...
        className={`flex-none mt-0.5 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors ${
          task.done
      ...
      <button
        ...
        title={task.inToday ? 'Прибрати з Сьогодні' : 'Додати до Сьогодні'}
      ...
      <button
        ...
        className="flex-none p-2 rounded-xl text-gray-300 active:text-red-400 active:bg-red-50 transition-colors"
        aria-label="Видалити"
      >
        <svg viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5">
          <path
      ...
      <div className="flex-1 px-4 space-y-2 pb-4">
        {inboxTasks.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-64 text-center gap-2">
            ...
            <p className="text-gray-500 font-medium">Inbox порожній</p>
